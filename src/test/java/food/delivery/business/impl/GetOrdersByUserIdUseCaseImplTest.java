...
class GetOrdersByUserIdUseCaseImplTest {
    ...
    @Test
    void getOrdersByUserId_shouldReturnOrdersConverted() {
        ...
        when(orderRepositoryMock.findAllByUser_Id(firstUserEntity.getId())).thenReturn(List.of(firstOrderEntity, secondOrderEntity));
        when(requestAccessToken.getUserId()).thenReturn(firstUserEntity.getId());

        GetOrdersResponse actualResult = getOrdersByUserIdUseCase.getOrdersByUserId(GetAllOrdersByUserIdRequest.builder().user_id(firstUserEntity.getId()).build());
        ...
        GetOrdersResponse expectedResult = GetOrdersResponse.builder().orders(List.of(firstOrder, secondOrder)).build();
        assertEquals(expectedResult, actualResult);

        verify(orderRepositoryMock).findAllByUser_Id(firstUserEntity.getId());
    }
}